...
nmap -T4 -p- $1 | grep "/" | cut -d " " -f 1 | tr -d "/tcpud" > port.txt #Runs nmap to scan all ports. It greps for line with '/' and uses delimiter to remove spaces and selects the 1st field and then truncates the characters and wites the output to port.txt
sed '1d' port.txt > ports.txt #Uses sed to remove 1st line as it was unnecessary characters
tr '\n' ',' < ports.txt > portnall.txt #Uses truncate to add comma so that the port number wont be in different line as it will rerun the code for each port
echo
...
echo
for ports in $(cat portnall.txt); do nmap -T4 -p $ports -A $1 > Results.txt #Runs the nmap -A script for the discovered ports
